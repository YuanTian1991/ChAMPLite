...
champ.prepareManifest <- function(manifest_path, skip_mark = "[Assay]") {
  manifest <- read.csv(manifest_path, head = TRUE, sep = ",")
  manifest <- manifest[, c(1, 3, 5, 7, 9)]

  type_II <- manifest[which(manifest$Infinium_Design_Type == "II"), ]
  type_I_Red <- manifest[which(manifest$Infinium_Design_Type == "I" &
    manifest$Color_Channel == "Red"), ]
  type_I.Grn <- manifest[which(manifest$Infinium_Design_Type == "I" &
    manifest$Color_Channel == "Grn"), ]
